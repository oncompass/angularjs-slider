...
angular.module('rzModule', [])
    ...
    .factory('Slider', ['$timeout', '$document', 'throttle', function ($timeout, $document, throttle) {
      ...
      Slider.prototype = {
        ...
        /**
         * Initializes the visible values
         *
         * @returns {undefined}
         */
        initVisibleSteps: function () {

          var self = this,
              minValue = this.scope.rzSliderFloor,
              maxValue = this.scope.rzSliderCeil,
              possibleSteps = [];

          if (!angular.isArray(this.scope.rzSliderShowSteps) && this.attributes.rzSliderShowSteps !== 'all') {
            return;
          }

          // Sanitizing Visible values
          if (angular.isArray(this.scope.rzSliderShowSteps)) {

            angular.forEach(this.scope.rzSliderShowSteps, function (value) {
              if (value > minValue && value < maxValue) {
                possibleSteps.push(value);
              }
            });

          } else {
            for (var i = minValue + this.step; i < maxValue; i += this.step) {
              possibleSteps.push(i);
            }
          }

          // Create visible step range from all
        ...
        initHandles: function () {
          ...
          if (angular.isArray(this.visibleSteps)) {
            angular.forEach(this.visibleSteps, function (stepValue, index) {
              var elem = this.stagLabs[index];
              this.updateVisibleStepHandle(this.roundStep(this.valueToOffset(stepValue)), stepValue, index);
          ...
          this.toggleVisibleStepHandlers();
        },
        ...
        initElemHandles: function () {
          angular.forEach(this.sliderElem.children(), function (elem, index) {
            ...
            switch (index) {
              ...
              case 8:
                ...
                break;
              default:
                this.stagLabs = this.stagLabs === null ? [] : this.stagLabs;
                this.stagLabs.push(_elem);
                break;
            }
          ...
          if (angular.isArray(this.visibleSteps)) {
            ...
            var stagLabs = this.stagLabs;

            angular.forEach(this.visibleSteps, function (step, key) {
              var stageLabel = stagLabs[key];
              stageLabel.rzsl = 0;
              stageLabel.stepValue = step;
            });
          }
        ...
        /**
         * Update visible step handle position and label
         *
         * @param {number} newOffset
         * @param {number} stepValue
         * @returns {undefined}
         */
        updateVisibleStepHandle: function (newOffset, stepValue, stepIndex) {
          var elem = this.stagLabs[stepIndex];
          this.setLeft(elem, newOffset - elem.rzsw / 2 + this.handleHalfWidth);
          this.translateFn(stepValue, elem);
        },

        /**
         * Update visible step handles based on value
         */
        toggleVisibleStepHandlers: function () {
          if (angular.isArray(this.visibleSteps)) {

            var modelValue = this.scope.rzSliderModel,
                modelHigh = this.scope.rzSliderHigh;

            angular.forEach(this.stagLabs, function (step, index) {
              if (this.visibleSteps[index] === modelValue || this.visibleSteps[index] === modelHigh) {
                this.hideEl(step);
              } else {
                this.showEl(step);
              }
        ...
        bindEvents: function () {
          ...
          if (angular.isArray(this.visibleSteps)) {
            angular.forEach(this.stagLabs, function (stageLabel) {
              stageLabel.on('click', angular.bind(this, this.onVisibleStageLabelClick));
            }, this);
          }

          this.fullBar.bind('mousedown touchstart', function (event) {
            var newValue = this.roundStep(this.offsetToValue(event.offsetX)),
                lowVal = this.scope.rzSliderModel,
                hiVal = this.scope.rzSliderHigh,
                modelValueKey = 'rzSliderModel';

            if (this.range && (hiVal <= newValue || (Math.abs(hiVal) - Math.abs(newValue)) < (Math.abs(newValue) - Math.abs(lowVal)))) {
              modelValueKey = 'rzSliderHigh';
            }

            this.onStart(modelValueKey === 'rzSliderModel' ? this.minH : this.maxH, modelValueKey, event);

          }.bind(this));

          this.flrLab.on('click', angular.bind(this, this.onMinMaxClick, 'rzSliderFloor'));
          this.ceilLab.on('click', angular.bind(this, this.onMinMaxClick, 'rzSliderCeil'));

        },

        /**
         * Sets the model to the terminus value of the slider
         *
         * @param {string} terminus
         * @returns {undefined}
         */
        onMinMaxClick: function (terminus) {

          var newValue = this.scope[terminus],
              modelKey = terminus === 'rzSliderCeil' ? (this.range ? 'rzSliderHigh' : 'rzSliderModel') : 'rzSliderModel',
              updateMethod = modelKey === 'rzSliderModel' ? 'updateLowHandle' : 'updateHighHandle';

          this.scope[modelKey] = newValue;
          this.scope.$apply();
          this[updateMethod](this.valueToOffset(newValue));
        ...
        onVisibleStageLabelClick: function (event) {

          var stepLab,
              newValue,
              newOffset,
              eventTarget = event.target,
              lowerModelValue = this.scope.rzSliderModel,
              higherModelValue = this.scope.rzSliderHigh,
              modelValueKey = 'rzSliderModel';

          angular.forEach(this.stagLabs, function (stagLab) {
            if (eventTarget === stagLab[0]) {
              stepLab = stagLab;
            }
          }, this);

          newValue = parseInt(stepLab.stepValue);
          newOffset = this.valueToOffset(newValue);


          if (lowerModelValue >= newValue) {
            modelValueKey = 'rzSliderModel';
          } else if (higherModelValue <= newValue) {
            modelValueKey = 'rzSliderHigh';
          } else {

            if (Math.abs(newValue - lowerModelValue) > Math.abs(higherModelValue - newValue)) {
              modelValueKey = 'rzSliderHigh';
            } else {
              modelValueKey = 'rzSliderModel';
            }
          ...
          this.scope[modelValueKey] = newValue;
          this.updateHandles(modelValueKey, newOffset);
          this.updateCmbLabel();
          this.updateSelectionBar();
          this.toggleVisibleStepHandlers();
          this.scope.$apply();
 ...
 * @property {number} rzSliderShowSteps
 */
